      ...
      <div className={ scroll ? "mxa-ha" : "mxa-ha md:px-20px xl:px-0" }>
        ...
        <div className={ scroll ? "lg:px-20px xl:px-0 transition-fast transition-timing-ease-out transition-property-bg transition-delay-long bg-white fixed h-64px lg:h-80px w-full top-0 z-1 shadow" : "mt-20px md:mt-0 h-39-80px lg:h-80px w-full"}>
          <div className={ scroll ? "container mx-auto pl-20px md:pl-40px md:pr-20px lg:pr-0 lg:pl-0 xl:px-0 j-grid items-center xl:w-1110px h-full" : "container mx-auto pl-20px lg:pl-0 j-grid items-center xl:w-1110px h-full"}>
            ...
            <button 
              className={ isExpanded ? "no-outline ab-inv f-ic-rdd mr-26-38px" : "no-outline md-ab-inv f-ic-rdd relative visible mr-26-38px" } 
              onClick={() => toggleExpansion(!isExpanded)}>
              <img src="/icons/icon-menu.svg" alt="show menu"/>
            </button>
            <button
              className={ isExpanded ? "no-outline md-ab-inv f-ic-rdd relative visible mr-26-38px text-24" : "no-outline ab-inv md-ab-inv flex rounded mr-26-38px text-24" }
              onClick={() => toggleExpansion(!isExpanded)}>
              &times;
            </button>
          </div>
        ...
        <div 
          ...
          <div className="c-grid w-auto h-auto mx-20px font-display text-14 text-black-1">
              <Link className="header-link no-outline" to="/#empresa" onClick={() => toggleExpansion(!isExpanded)} ><img src="icons/icon-play.svg" alt="icon list"/>&nbsp;Empresa</Link>
              <Link className="header-link no-outline" to="/#servicos" onClick={() => toggleExpansion(!isExpanded)}><img src="icons/icon-play.svg" alt="icon list"/>&nbsp;Serviços</Link>
              <Link className="header-link no-outline" to="/#clientes" onClick={() => toggleExpansion(!isExpanded)} ><img src="icons/icon-play.svg" alt="icon list"/>&nbsp;Clientes</Link>
              <Link className="header-link no-outline" to="/#testemunho" onClick={() => toggleExpansion(!isExpanded)} ><img src="icons/icon-play.svg" alt="icon list"/>&nbsp;Testemunho</Link>
              <Link className="header-link no-outline" to="/#talentos" onClick={() => toggleExpansion(!isExpanded)} ><img src="icons/icon-play.svg" alt="icon list"/>&nbsp;Talentos</Link>
              <Link className="header-link no-outline" to="/#contato" onClick={() => toggleExpansion(!isExpanded)} ><img src="icons/icon-play.svg" alt="icon list"/>&nbsp;Contato</Link>
